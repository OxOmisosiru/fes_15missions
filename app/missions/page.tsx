...
export default async function MissionList() {
  ...
  return <main>
    <header className="header">
      <div>
        <div className="brand">15 ミッション一覧</div>
    ...
    <div className="grid">
      {missions.map((mission) => {
        const status = byIndex.get(mission.index);
        return <Link
          className="mission" href={`/missions/${mission.index}`} key={mission.index}>
            <strong>{mission.title}</strong>
            <div className={status?.is_cleared ? "status cleared" : "status"}>
              {status?.is_cleared ? "✓ クリア済" : "未クリア"}
            </div>
        </Link>; })}
    </div>
  </main>;
